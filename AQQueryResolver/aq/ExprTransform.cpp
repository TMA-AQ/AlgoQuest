								...
								int *pErr ) 
{
  ...
  if (this->baseDesc.getTable(nTableId)->isTemporary())
  {
    if (!thesaurus.Items.empty())
    {
    ...
    try 
    {
      aq::TemporaryColumnMapper::Ptr colMapper(new aq::TemporaryColumnMapper(settings.szTempPath1, nTableId, nColumnId, eColumnType, nColumnSize, this->settings.packSize));
      thesaurus.loadFromTmp( eColumnType, colMapper );
      return 0;
    }
    catch (const aq::generic_error&)
    {
    }
  }
  else
  {
    ...
    std::string fileName = aq::getThesaurusFileName( settings.szThesaurusPath, nTableId, nColumnId, nPartNumber );

    if ( thesaurus.loadFromThesaurus( fileName.c_str(), 0, nColumnSize, eColumnType, pErr ) != 0 ) 
    {
      // Try Text Version Next !
      fileName += ".txt";
      if ( thesaurus.loadFromThesaurus( fileName.c_str(), 1, nColumnSize, eColumnType, pErr ) != 0 ) 
      {
...
//------------------------------------------------------------------------------
// Return -1 on error !
int ExpressionTransform::get_thesaurus_info_for_column_reference( aq::tnode *pNode,
											...
											ColumnType *peColumnType, int *pErr ) 
{
	...
	if ( get_table_and_column_id_from_table_array(	&baseDesc, pNode->left->getData().val_str,
		pNode->right->getData().val_str, 
...
// 
//------------------------------------------------------------------------------
aq::tnode* ExpressionTransform::transform_between( aq::tnode* pNode, int *pErr ) {
	int			bNotBetween;
	aq::tnode		*pNodeTmp;
	...
	if ( get_thesaurus_info_for_column_reference( pNodeColumnRef, &nTableId, &nColumnId, &nColumnSize, &eColumnType, pErr ) == -1 )
		return pNode;
	...
	for ( nLoopCnt = 0; nLoopCnt < 1000; nLoopCnt++ ) {
		...
		if ( get_thesaurus_for_column( thesaurus, nTableId, nColumnId, nColumnSize, nLoopCnt, eColumnType, pErr ) != 0 ) {
...
// 
//------------------------------------------------------------------------------
aq::tnode* ExpressionTransform::transform_like( aq::tnode* pNode, int *pErr ) {
	int			bNotLike;
	aq::tnode		*pNodeTmp;
	aq::tnode		*pNodeColumnRef;
	aq::tnode		*pNodeStr;
...
// 
//------------------------------------------------------------------------------
aq::tnode* ExpressionTransform::transform_cmp_op( aq::tnode* pNode, int *pErr ) {
	aq::tnode		*pNodeColumnRef;
	aq::tnode		*pNodeStr;
	aq::tnode		*pNodeRes;
	Column		thesaurus;
...
//------------------------------------------------------------------------------
ExpressionTransform::ExpressionTransform(Base& _baseDesc, TProjectSettings& _settings)
  : 
  baseDesc(_baseDesc),
  settings(_settings)
{
...
//------------------------------------------------------------------------------
int ExpressionTransform::get_thesaurus_for_column_reference( Column& thesaurus, aq::tnode *pNode, int part, int *pErr ) {
	unsigned int	nTableId;
	unsigned int	nColumnId;
	unsigned int	nColumnSize;
	ColumnType		eColumnType;

	if ( get_thesaurus_info_for_column_reference( pNode, &nTableId, &nColumnId, &nColumnSize, &eColumnType, pErr ) == -1 )
		return -1;

	return get_thesaurus_for_column( thesaurus, nTableId, nColumnId, nColumnSize, part, eColumnType, pErr );
}


//------------------------------------------------------------------------------
aq::tnode* ExpressionTransform::expression_transform( aq::tnode *pNode, int *pErr ) {
	if ( pErr == NULL )
		return pNode;
	if ( pNode == NULL )
		return NULL;
	...
	// Call recursively
	pNode->next = expression_transform( pNode->next, pErr );
	pNode->left = expression_transform( pNode->left, pErr );
	// Do not call on K_PERIOD's node right branch if the right tag is K_COLUMN !
	if ( pNode->right != NULL ) {
		if ( pNode->tag != K_PERIOD || pNode->right->tag != K_COLUMN ) {
			pNode->right = expression_transform( pNode->right, pErr );
		}
	}
