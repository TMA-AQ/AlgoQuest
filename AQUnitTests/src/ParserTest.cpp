...
BOOST_AUTO_TEST_CASE(big_boss)
{
  ...
  std::string expected = " SELECT  T1  .  A  ,  T1  .  B  ,  TMP2  .  A  ,  TMP2  .  D  ,  TMP2  .  E  ,  T1  .  A  +  T1  .  B  -  TMP2  .  E  SUM  AS  v_sum  ,  T1  .  A  CASE  1  WHEN  T1  .  B  2  WHEN  TMP2  .  E  AS  t1a_case  FROM  T1  ,  T2  AS  TMP2  ,  SELECT  T1  .  A  FROM  T1  WHERE  T1  .  A  =  T1  .  B  AND  T1  .  A  <  10  AND  T1  .  B  IN  1  ,  2  ,  3  ORDER  T1  .  A    AS  TMP3  ,  SELECT  T2  .  A  ,  T2  .  D  SUM  AS  AA  FROM  T2  WHERE  T2  .  A  =  T2  .  E  GROUP  T2  .  A  ,  T2  .  E    AS  TMP4  WHERE  T1  .  A  =  TMP4  .  A  AND  T1  .  B  =  TMP4  .  D  AND  TMP2  .  D  =  TMP2  .  E  AND  TMP4  .  D  ,  4  ,  3  SUBSTRING  =  T1  .  B  AND  TMP3  .  A  =  TMP4  .  AA  GROUP  T1  .  A  ,  T1  .  B  ,  TMP2  .  A  ,  TMP4  .  D  ,  TMP2  .  E    HAVING  v_sum  <  10  ORDER  TMP2  .  E   ";
  std::string baseFilename = "base_test.aq";
  runParserTest( query, expected, baseFilename );
}
...
BOOST_AUTO_TEST_CASE(max_simple)
{
  ...
  std::string expected = " SELECT  T1  .  A  MAX  AS  TEST  FROM  T1 ";
  std::string baseFilename = "base_test.aq";
  runParserTest( query, expected, baseFilename );
}
...
BOOST_AUTO_TEST_CASE(max_as_simple)
{
  ...
  std::string expected = " SELECT  TMP  .  A  MAX  AS  TEST  FROM  T1  AS  TMP ";
  std::string baseFilename = "base_test.aq";
  runParserTest( query, expected, baseFilename );
}
...
BOOST_AUTO_TEST_CASE(column_as_sum_simple)
{
  ...
  std::string expected = " SELECT  TMP2  .  AA  FROM  SELECT  TMP1  .  B  SUM  AS  AA  FROM  T1  AS  TMP1  AS  TMP2 ";
  std::string baseFilename = "base_test.aq";
  runParserTest( query, expected, baseFilename );
}
...
BOOST_AUTO_TEST_CASE(partition_by)
{
  ...
  std::string expected = " SELECT  T1  .  A  ,  T1  .  A  SUM  ORDER  T1  .  B    T1  .  A    PARTITION  FROM  T1 ";
  std::string baseFilename = "base_test.aq";
  runParserTest( query, expected, baseFilename );
}
