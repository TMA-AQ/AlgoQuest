...
int MakeBackupFile( char *pszPath, int nIdx, int type )
{
	...
	if( FileCopy( pszPath, szDstPath ) != 0 )
	{
#ifdef CREATE_LOG
...
//------------------------------------------------------------------------------
QueryResolver::QueryResolver(TProjectSettings * _pSettings, AQEngine_Intf * _aq_engine, Base& _baseDesc)
...
QueryResolver::~QueryResolver()
{
	...
	oss << "Query Resolver Time elapsed: " << (end - this->begin);
...
VerbNode::Ptr QueryResolver::BuildVerbsSubtree(	tnode* pSelect, tnode* pStart, tnode* pStartOriginal, int context )
{
...
//------------------------------------------------------------------------------
VerbNode::Ptr QueryResolver::BuildVerbsTree( tnode* pStart )
{
	...
	for( size_t idx = 0; idx < categories.size(); ++idx )
	{
		...
		VerbNode::Ptr spNode = this->BuildVerbsSubtree( pStart, pNode, pStartOriginal, categories[idx] );
...
//------------------------------------------------------------------------------
void QueryResolver::getColumnTypes( tnode* pNode, vector<Column::Ptr>& columnTypes )
{
	if( !pNode || !pNode->left )
		return;
...
//-------------------------------------------------------------------------------
Table::Ptr QueryResolver::SolveSelectRegular( tnode *pNode, int nSelectLevel )
{
...
// 	sprintf( szBuffer, "%s %s", pSettings->kjeqParser, pSettings->szOutputFN );
// 	system( szBuffer );
	...
	// 
	begin = (boost::posix_time::microsec_clock::local_time());
	VerbNode::Ptr spTree = this->BuildVerbsTree( pNode );
	// TODO: optimize tree by detecting identical subtrees
	spTree->changeQuery();
	cleanQuery( pNode );
	end = (boost::posix_time::microsec_clock::local_time());
	oss.str("");
	oss << "Build Verbs Tree time elapsed: " << (end - begin);
	aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());

	//
	//
	begin = (boost::posix_time::microsec_clock::local_time());
	Table::Ptr table = solveOptimalMinMax( spTree, BaseDesc, *pSettings );
	end = (boost::posix_time::microsec_clock::local_time());
	oss.str("");
	oss << "Solve Optimal MinMax time elapsed: " << (end - begin);
	aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());
	if( table )
		return table;
	...
	this->getColumnTypes( pNode, columnTypes );
	end = (boost::posix_time::microsec_clock::local_time());
	oss.str("");
	oss << "Get Column Types time elapsed: " << (end - begin);
	aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());
	...
	if( aq_engine->call( *this->pSettings, pNode, 0, nSelectLevel ) != 0 )
	{
		oss.str("");
		oss << "error calling AQEngine";
    ...
    std::vector<std::string> answerFiles;
		answerFiles.push_back(pSettings->szAnswerFN);

		//
		//
		begin = (boost::posix_time::microsec_clock::local_time());
		table->loadFromAnswer( answerFiles, pSettings->fieldSeparator, 
			columnTypes, *pSettings, BaseDesc );
		end = (boost::posix_time::microsec_clock::local_time());
		oss.str("");
		oss << "Load answer time elapsed: " << (end - begin);
		...
		if( !table->NoAnswer )
		{
			...
			oss.str("");
			oss << "Change Result time elapsed: " << (end - begin);
			aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());
		}
		...
		oss.str("");
		oss << "Clean RedundantColumns time elapsed: " << (end - begin);
		aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());
		...
		oss.str("");
		oss << "Group by time elapsed: " << (end - begin);
		aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());

#ifdef OUTPUT_NESTED_QUERIES
		table->saveToAnswer( pSettings->szAnswerFN, pSettings->fieldSeparator );
		MakeBackupFile( pSettings->szAnswerFN, nQueryIdx, 0 );
#endif

		begin = boost::posix_time::microsec_clock::local_time();
		spTree.reset(); //debug13 - force delete to see if it causes an error
		end = boost::posix_time::microsec_clock::local_time();
		oss.str("");
		oss << "Clean up time elapsed: " << (end - begin);
		aq::Logger::getInstance().log(AQ_INFO, "%s\n", oss.str().c_str());
	}
	return table;
}
							...
							int nSelectLevel)
{
	...
	std::string queryStr;
	syntax_tree_to_sql_form(pInteriorSelect, queryStr);
	aq::Logger::getInstance().log(AQ_INFO, "\n\n%s\n\n", queryStr.c_str());

	solveSelectStar( pInteriorSelect, BaseDesc );
	// solveOneTableInFrom( pInteriorSelect, BaseDesc ); // this is done by the changeQuery on VerbNode
	...
	VerbNode::Ptr spTree = this->BuildVerbsTree( pInteriorSelect );
	spTree->changeQuery();
	cleanQuery( pInteriorSelect );

	//DumpVisitor * dv = new DumpVisitor();
	//spTree->accept(dv);
	//aq::Logger::getInstance().log(AQ_DEBUG, "%s\n", dv->getQuery().c_str());

	aq_engine->call( *this->pSettings, pInteriorSelect, minMaxGroupBy ? 0 : 1, nSelectLevel);
											...
											bool inIn )
{
	...
	if( inFrom )
	{
		...
		std::string queryStr;
		syntax_tree_to_sql_form(pLastSelect, queryStr);
		aq::Logger::getInstance().log(AQ_INFO, "\n\n%s\n\n", queryStr.c_str());

	}
	else
	{
		Table::Ptr table = SolveSelectRegular( pNode, nSelectLevel );
...
//-------------------------------------------------------------------------------
Table::Ptr QueryResolver::SolveSelect( tnode* pNode )
{
	assert( pNode->tag == K_SELECT );
	SolveSelectRecursive( pNode, 0, NULL, false, false );
	return SolveSelectRegular( pNode, 1 );
}
						...
						int endIdx, bool append )
{
	...
	sprintf( szBuffer, "\"%s %u %u %u\"\n", 
		pSettings->szLoaderPath, tableIdx + 1, colIdx + 1, packNumber );
	system ( szBuffer );
}
...
void QueryResolver::SolveInsert(	tnode* pNode )
{
	...
	{
		valuesToInsert = SolveSelect( pNode->right->right );
...
void QueryResolver::SolveUpdateDelete(	tnode* pNode )
{
	...
	if( conditionsRoot->tag == K_IN )
	{
		...
		condTable = SolveSelect( pNode->right->right );
	...
	for( size_t packNr = 0; packNr < (size_t) nrOfPacks; ++packNr )
	{
		...
		if( atLeastOneUpdate )
		{
			for( size_t idx = 0; idx < updates.size(); ++idx )
			{
				...
				sprintf( szBuffer, "\"%s %d %d %u\"\n", 
					pSettings->szLoaderPath, tableIdx + 1, 
...
void QueryResolver::SolveUnionMinus(	tnode* pNode )
{
	...
	{
		Table::Ptr table = SolveSelect( queries[idx] );
...
void QueryResolver::SolveCreate(	tnode* pNode )
{
	...
	{
		sprintf( szBuffer, "cmd /s /c \"%s \"%u\" \"%u\"\"", pSettings->szCutInColPath, 
...
//------------------------------------------------------------------------------
int QueryResolver::SolveSQLStatement(	tnode *pNode )
{
	if( !pNode )
		throw generic_error(generic_error::INVALID_QUERY, "");
	...
	case K_SELECT:
		{
			Table::Ptr table = SolveSelect( pNode );
			if ((table != 0) && (pSettings->computeAnswer))
			{
				table->saveToAnswer( pSettings->szAnswerFN, pSettings->fieldSeparator );
			}
			// exit( 0 ); //let the OS reclaim memory
		}
		break;
	case K_CREATE:
		{
